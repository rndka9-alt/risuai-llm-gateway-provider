...
export const STYLES = [
  ...
  '.flags { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:9px 12px; margin:0; padding:12px; border:1px solid var(--border2); border-radius:7px; }',
  '.flags legend { padding:0 5px; color:var(--text2); font-size:.85em; }',
  '.checkbox { display:flex; align-items:center; gap:7px; min-width:0; font-size:.85em; }',
  '.checkbox input { width:auto; margin:0; padding:0; accent-color:var(--accent); }',
  '.checkbox.unsupported { color:var(--text2); opacity:.7; }',
  '.notice { margin:0; padding:9px 11px; border:1px solid var(--accent); border-radius:7px; color:var(--text); font-size:.85em; }',
  '.notice[hidden] { display:none; }',
  '.ledger { display:flex; align-items:center; flex-wrap:wrap; gap:6px; font-size:.85em; color:var(--text2); }',
  '.ledger .amount.gain { color:#4ade80; }',
  '.ledger .amount.loss { color:#f87171; }',
  '.ledger button { min-width:0; padding:1px 7px; font-size:.9em; line-height:1.4; }',
