...
import { neutralScenarios } from './authored-scenario-fixtures';
...
export function createAuthoredNeutralScenarios(): readonly WeightedScenario[] {
  return neutralScenarios.map((scenario) => ({
    scenario: {
      ...
      requests: scenario.requests.map((request) => ({
        ...
        messages: request.messages.map((message): LlmMessage => ({
          role: message.role,
