...
export function createMultiRoomRoundRobinScenario(): SimulationScenario {
...
export function createContentAddressedRoundRobinScenario(): SimulationScenario {
  const rooms: Array<{
    head: LlmMessage;
    lorebook: LlmMessage;
    persona: LlmMessage;
...
// entry 생존을 구분한다.
export function createCrossChurnEvictionScenario(): SimulationScenario {
  ...
  if (activeRoom === undefined || longIdleRoom === undefined) {
    throw new Error('Cross-churn scenario requires two normal rooms.');
  }
  let activeTurn = 6;
